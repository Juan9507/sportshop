...
public interface ProductoRepositorio extends R2dbcRepository<ProductoTabla,Integer> {
    ...
    @Query("""
            ...
            LEFT JOIN CATEGORIA categoria ON categoria.ID_CATEGORIA = producto.CATEGORIA_ID_CATEGORIA
            LEFT JOIN GENERO genero ON genero.ID_GENERO = producto.GENERO_ID_GENERO
            LEFT JOIN ESTADO estado ON estado.ID_ESTADO = producto.ESTADO_ID_ESTADO
            LEFT JOIN PRODUCTO_TIENE_TALLA tallas ON tallas.PRODUCTO_COD_PRODUCTO = producto.CODIGO_PRODUCT
            LEFT JOIN TALLA talla ON talla.ID_TALLA = tallas.TALLA_ID_TALLA
    ...
    @Query("""
            ...
            LEFT JOIN PRODUCTO_TIENE_TALLA tallas ON tallas.PRODUCTO_COD_PRODUCTO = producto.CODIGO_PRODUCT
            LEFT JOIN TALLA talla ON talla.ID_TALLA = tallas.TALLA_ID_TALLA
            WHERE CODIGO_PRODUCT = :id
            GROUP BY CODIGO_PRODUCT, NOMBRE,CANTIDAD,PRECIO,CATEGORIA, GENERO, ESTADO;
            """)
    Mono<ProductoTabla> consultarProductoConId(
            @Param("id") Integer id
    );
}
